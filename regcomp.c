...
#endif
    I32 paren = 0;
    regnode *start = RExC_rxi->program + 1;
    regnode *end = RExC_opend;
    U32 my_recursed_depth = params->recursed_depth;

    PERL_ARGS_ASSERT_RCK_GOSTART;
...
#define TRIE_TYPE(X) ( ( NOTHING == (X) ) ? NOTHING :   \
           ...
           0 )
            ...
            DEBUG_TRIE_COMPILE_r({
                regprop(RExC_rx, RExC_mysv, cur, NULL, pRExC_state);
                PerlIO_printf(Perl_debug_log, "%*s- %s (%d) <SCAN FINISHED>\n",
                    (int)params->depth * 2 + 2, "",
                    SvPV_nolen_const(RExC_mysv), REG_NODE_NUM(cur)
                );
            });
...
STATIC bool S_study_chunk_one_node(pTHX_ RExC_state_t *pRExC_state, rck_params_t *params)
{
    ...
    } else if (OP(params->scan) == GOSTART) {
        return rck_gostart(pRExC_state, params);
