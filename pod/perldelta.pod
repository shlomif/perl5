...
it to be executed immediately when it is closed over its outer variables.
Its value is captured and used to create a new constant subroutine that is
returned.  This feature is experimental.  See L<perlsub/Constant Functions>.
...
Pathtools no longer tries to load XS on miniperl.
...
precision floating point numbers on x86-32, x86-64, and IA-64 platforms.  See
...
NeXTSTEP was a proprietary operating system bundled with NeXT's workstations
in the early to mid 90s; OPENSTEP was an API specification that provided a
NeXTSTEP-like environment on a non-NeXTSTEP system.  Both are now long dead,
so support for building Perl on them has been removed.
...
XXX In release 5.21.8, compiling on Visual C++ with dmake was broken. This is
now fixed.
...
Previously, on Visual C++ for Win64 built Perls only, compiling every Perl
...
warnings have been silenced.  32-bit and 64-bit GCCs and Visual C++ for Win32
were not affected.
...
undocumented and has no usage in modern perl versions (or in CPAN modules,
based on a CPAN search). Calling it has been fatal since 5.17.0.
...
C<Perl_tmps_grow> which is marked as public API, but an undocumented one,
has been removed from being public API. If you use C<EXTEND_MORTAL> macro in
your XS code to preextend the mortal stack, you are unaffected by this change.
...
and
C<[[:xdigit:]]>.
These are so because the underlying Microsoft implementation does not
